import type { CoinId, MarketSnapshot, RoleId } from "../types";
import { localISODate } from "../utils/format";

export const COIN_META: Record<CoinId, { name: string; color: string; providerId: string }> = {
  BTC: { name: "Bitcoin", color: "#efae70", providerId: "bitcoin" },
  ETH: { name: "Ethereum", color: "#9aabf5", providerId: "ethereum" },
  BNB: { name: "BNB", color: "#e5c871", providerId: "binancecoin" },
  XRP: { name: "XRP", color: "#a6ced1", providerId: "ripple" },
  SOL: { name: "Solana", color: "#acdf9e", providerId: "solana" },
};
...
// These fixed example values are never presented as live market prices.
