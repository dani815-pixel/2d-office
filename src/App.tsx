...
export default function App() {
  ...
  const [importStatus, setImportStatus] = useState<"READY" | "PARSING" | "VALID" | "WARNING" | "INVALID">("READY");
  ...
  const handleImport = () => {
    ...
    window.setTimeout(() => {
      ...
      setPreview(result); setImportStatus(result.brief ? (result.warnings.length ? "WARNING" : "VALID") : "INVALID");
    }, 180);
  };
  const confirmImport = () => {
      ...
      <main className="page-content">
        ...
        {view === "IMPORT" && <div className="standard-page import-page">
          ...
          {preview && <section className={`preview-panel ${preview.brief ? "preview-panel--valid" : "preview-panel--invalid"}`}>
            <div className="preview-header"><div><span className="eyebrow-small">IMPORT PREVIEW / {preview.format}</span><h2>{preview.brief ? (preview.warnings.length ? "Market Brief is ready with warnings." : "Market Brief is ready.") : "결과를 확인해 주세요."}</h2></div><span className={`preview-stamp ${preview.brief ? (preview.warnings.length ? "preview-stamp--warning" : "preview-stamp--valid") : "preview-stamp--invalid"}`}><Icon name={preview.brief ? (preview.warnings.length ? "clock" : "check") : "import"} size={15} /> {preview.brief ? (preview.warnings.length ? "WARNING" : "VALID") : "INVALID"}</span></div>
