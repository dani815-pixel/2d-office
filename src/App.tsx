    ...
    <div className="main-shell">
      ...
      <main className="page-content">
        ...
        {view === "OFFICE" && <div className="office-page">
          ...
          <div className="scene-meta"><span><span className="meta-accent">SOURCE</span> {app.market.status === "MOCK" ? "DEMO DATA / USD" : "CoinGecko / USD"} {app.market.status === "MOCK" && <b>· 고정 예시 데이터, 실시간 아님</b>}</span><span>{app.market.status === "MOCK" ? "SAMPLE DATE" : "LAST SYNC"} {app.market.status === "MOCK" ? dateText(app.market.timestamp) : timeText(app.market.timestamp)} {app.market.status === "STALE" && "/ CACHED"}</span></div>
        ...
        {view === "MARKET" && <div className="standard-page market-page"><PageHead eyebrow="02 / MARKET INTELLIGENCE" title={<>Market <em>monitor.</em></>} description="5개 자산의 가격과 거래량을 한 번의 API 요청으로 확인합니다." action={<button className="button button--outline" type="button" onClick={refreshMarket} disabled={marketLoading}><Icon name="refresh" size={16} className={marketLoading ? "spin" : ""} />데이터 갱신</button>} /><div className="section-rule"><span><i className={`feed-dot feed-dot--${app.market.status.toLowerCase()}`} />{marketLoading ? "CONNECTING TO COINGECKO" : `${app.market.status} MARKET DATA`}<span className="section-rule-muted"> / COINGECKO · USD</span></span><span>UPDATED {dateText(app.market.timestamp)} · {timeText(app.market.timestamp)}</span></div>{marketError && <div className="inline-warning"><span>!</span>{marketError}</div>}<div className="market-grid">{app.market.coins.map((coin, index) => <button type="button" key={coin.id} onClick={() => setSelectedCoin(coin.id)} className={`coin-card ${selectedCoin === coin.id ? "coin-card--selected" : ""}`}><div className="coin-card-top"><span className="coin-symbol" style={{ color: COIN_META[coin.id].color, borderColor: `${COIN_META[coin.id].color}70`, background: `${COIN_META[coin.id].color}18` }}>{coin.id.slice(0, 1)}</span><span className="coin-card-name"><strong>{COIN_META[coin.id].name}</strong><small>{coin.id} / USD</small></span><span className="coin-index">0{index + 1} / 05</span></div><div className="coin-card-price">{priceUSD(coin.price)}</div><div className={`coin-card-change ${coin.change24h >= 0 ? "up" : "down"}`}>{coin.change24h >= 0 ? "↗" : "↘"} {changeText(coin.change24h)} <span>24H</span></div><div className="coin-card-stats"><div><span>24H VOLUME</span><strong>{compactUSD(coin.volume24h)}</strong></div><div><span>MARKET CAP</span><strong>{coin.marketCap ? compactUSD(coin.marketCap) : "N/A"}</strong></div></div><div className="coin-card-bottom"><span><i className={`feed-dot feed-dot--${app.market.status.toLowerCase()}`} />{app.market.status} DATA</span><span>{timeText(app.market.timestamp)} <Icon name="arrow" size={14} /></span></div></button>)}</div><div className="market-insight"><div className="insight-header"><span className="eyebrow-small">SELECTED ASSET / {marketCoin.id}</span><h2>{COIN_META[marketCoin.id].name}<span> / {marketCoin.id}</span></h2></div><div className="insight-columns"><div><span className="data-type">01 / FACT · MARKET API</span><p>{priceUSD(marketCoin.price)} <span className={marketCoin.change24h >= 0 ? "up" : "down"}>({changeText(marketCoin.change24h)} / 24h)</span></p><small>가격과 거래량은 AI 결과가 아닌 {app.market.status === "MOCK" ? "고정 예시 데이터" : "CoinGecko 스냅샷"}를 기준으로 합니다.</small></div><div><span className="data-type">02 / INTERPRETATION · AI RESEARCH</span><p>{coinInterpretation?.summary || "아직 가져온 AI 분석이 없습니다."}</p><small>{coinInterpretation ? "AI 해석은 가격 사실 데이터를 변경하지 않습니다." : "조사 결과를 가져오면 이곳에 해석이 표시됩니다."}</small></div></div></div></div>}
        ...
        {view === "IMPORT" && <div className="standard-page import-page">
          ...
          {preview && <section className={`preview-panel ${preview.brief ? "preview-panel--valid" : "preview-panel--invalid"}`}>
            ...
            {preview.brief && <div className="preview-confirm"><span>FACT와 INTERPRETATION은 별도로 보관됩니다.</span><button className="button button--primary" type="button" onClick={confirmImport}><Icon name="check" size={17} /> CONFIRM IMPORT <Icon name="arrow" size={17} /></button></div>}
          </section>}
        </div>}
        ...
        {view === "MEETING" && <div className="standard-page meeting-page">
          ...
          {!app.brief ? <EmptyState index="05" title="회의 자료가 아직 없습니다." description="AI 조사 결과를 가져오고 미리보기를 확인한 뒤 회의를 시작할 수 있습니다." button="AI 결과 가져오기" onClick={() => go("IMPORT")} /> : <>
            <div className="meeting-layout"><div className="meeting-stage">
              <Office market={app.meeting.snapshot || app.market} brief={app.brief} meeting={app.meeting} onCoinClick={openCoin} />
